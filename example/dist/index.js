...
function ZoomImage({ fullPath, onClose, onPrevious, onNext }) {
  const { data } = usePromise(fullPath);
...
function ImageThumbnail({ url, onClick }) {
  const { data } = usePromise(url);
  return data && jsx_dev_runtime2.jsxDEV("div", {
    style: {
      width: 100,
      height: 100,
...
function ImageList({ tree, path, u }) {
  ...
  return jsx_dev_runtime3.jsxDEV(jsx_dev_runtime3.Fragment, {
    children: images?.[zoomIndex] ? jsx_dev_runtime3.jsxDEV(jsx_dev_runtime3.Fragment, {
      ...
      children: images?.map((url, index) => jsx_dev_runtime3.jsxDEV(ImageThumbnail, {
        url: u(url),
